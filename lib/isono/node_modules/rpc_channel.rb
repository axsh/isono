...
module Isono
  module NodeModules
    class RpcChannel < Base
      ...
      initialize_hook do
        ...
        amq.queue("command-recv.#{manifest.node_id}", {:exclusive=>true}).subscribe { |header, data|
          ...
          if req
            ...
            event.publish('rpc/response_received', :args=>[manifest.node_id, data])
            begin
              case data[:type]
              when :inprogress
      ...
      # @param [Array] args
      # @param [Proc] &blk Setup the request context in async mode.
      # @return [AsyncRequestContext,any]
      #
      ...
      def request(endpoint, command, *args, &blk)
        if blk
          ...
          req = SyncRequestContext.new(endpoint, command, args)
        end
      ...
      # Make synchronous RPC request to the endpoint.
      #
      # @param [String] endpoint
      # @param [String] command
      # @param [Array] args
      ...
      # @exmaple
      # req = sync_request('endpoint', 'command', xxxx)
      # puts req.wait # The result from endpoint/command is shown.
      def sync_request(endpoint, command, *args)
        req = SyncRequestContext.new(endpoint, command, args)
        send_request(req)
        req
      end
      ...
      # @param [AsyncRequestContext] req Request context object to be
      # sent. If the context's state is not :init, it will fail.
      def send_request(req)
        raise TypeError if !req.is_a?(AsyncRequestContext)
      ...
      class ResponseContext
        ...
        def response(ret)
          ...
          EM.schedule {
            ...
            EventChannel.new(@node).publish('rpc/response_sent', :args=>{@node.node_id, ret})
          }
          @responded = true
        end
      ...
      class SyncRequestContext < AsyncRequestContext
        def initialize(endpoint, command, args)
