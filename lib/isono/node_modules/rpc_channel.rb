    ...
    class RpcChannel < Base
      ...
      def register_endpoint(endpoint, dispatcher)
        ...
        amq.queue(endpoint_queue_name(endpoint), {:exclusive=>true}).subscribe(:ack=>true) { |header, data|
          ...
          begin
            ret = dispatcher.dispatch(data[:command], *data[:args])
          rescue Exception => e
            logger.error(e)
            ret = e
          ensure
            header.ack
            unless data[:oneshot]
              msg = if ret.is_a? Exception
                      {:type => :error, :body=>{:message=> e.message, :error_type => e.class.to_s}}
                    else
                      {:type=> :success, :body => ret}
                    end
              amq.queue(header.reply_to).publish(Serializer.instance.marshal(msg),
                                                 {:key=>header.reply_to,
                                                   :message_id=>header.message_id}
                                                 )
              event.publish('rpc/response_sent', manifest.node_id, msg)
            end
          end
        }
      ...
      class AsyncRequestContext < OpenStruct
        ...
        def initialize(endpoint, command, args)
          ...
          @error_cb = nil
          @timer = nil
          @sent_at = @received_at = nil

          @stm = Statemachine.build {
            trans :init, :on_ready, :ready
            trans :ready, :on_sent, :waiting, proc { @sent_at=Time.now}
            trans :waiting, :on_received, :done, proc { @received_at=Time.now}
          }
          @stm.context = self
        end
        ...
        def hash
          ...
          # not pushed in @table.
          @table.dup.merge({:state=>self.state, :sent_at=>@sent_at, :received_at=>@received_at})
        end
